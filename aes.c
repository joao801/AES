...
int K, Nb = 4, Nk, Nr;
uint8_t mix[] = {0x02, 0x01, 0x01, 0x03}, inv_mix[] = {0x0e, 0x09, 0x0d, 0x0b}; 
...
void add_round_key(uint8_t *state, uint8_t *w, uint8_t r) {
	...
	for (c = 0; c < Nb; c++) {
		for(k=0; k<Nb; k++){
			state[Nb*k+c] ^= w[4*Nb*r+Nb*c+k];
		}
	}
}


void mix_columns(uint8_t *state, uint8_t * a) {
...
void sub_bytes(uint8_t *state, uint8_t *box) {
